...
"color: rgb(255, 255, 255);\n""")
        ...
        self.pushButton.clicked.connect(self.validate_credentials)
        self.lineEdit_2.returnPressed.connect(self.validate_credentials)  # Handle returnPressed signal
    ...
    def show_new_gui(self):
        ...
        with open("Employee_Passwords.txt", "r") as file:
            for line in file:
                ...
                if username == stored_username and entered_hashed_password == stored_hashed_password:
                    self.new_gui = QtWidgets.QWidget()
                    self.ui = Ui_Form1()
                    self.ui.setupUi(self.new_gui)
                    Form.close()
                    self.new_gui.show()
